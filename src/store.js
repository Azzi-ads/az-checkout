...
function defaults() {
  return {
    products: [],
    profile: { name: getUser()?.name || '', cpf: getUser()?.cpf || '', phone: getUser()?.phone || '', avatar: '', security: false, domain: '', plan: 'start', card: null, notifPending: true, notifPaid: true, notifSummary: true, notifMode: 'auto', notifTextPaid: '', notifTextPending: '', notifColor: '', fraudConfig: null, tracking: null },
...
export async function hydrate() {
  ...
  try {
    ...
    if (prof) store.profile = { ...store.profile, name: prof.name || store.profile.name, cpf: prof.cpf || getUser()?.cpf || '', phone: prof.phone || getUser()?.phone || '', avatar: prof.avatar || '', security: !!prof.security, domain: prof.domain || '', plan: prof.plan || 'start', notifPending: prof.notif_pending !== false, notifPaid: prof.notif_paid !== false, notifSummary: prof.notif_summary !== false, notifMode: prof.notif_mode || 'auto', notifTextPaid: prof.notif_text_paid || '', notifTextPending: prof.notif_text_pending || '', notifColor: prof.notif_color || '', fraudConfig: prof.fraud_config || null, tracking: prof.tracking || null }
...
async function pushProfile(profile) {
  ...
  try {
    await supabase.from('profiles').upsert({
      ...
      notif_pending: profile.notifPending !== false, notif_paid: profile.notifPaid !== false, notif_summary: profile.notifSummary !== false,
      ...
      fraud_config: profile.fraudConfig || null, tracking: profile.tracking || null,
    })
  } catch { /* segue só local */ }
}
